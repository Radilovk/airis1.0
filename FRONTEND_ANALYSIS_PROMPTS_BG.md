...
- **Минава ли моделът през всички 7 стъпки преди финалния JSON?** Да. `runMultistepPipeline` извиква последователно `step1`, `step2A`, `step2B`, `step2C`, `step3`, `step4` и `step5`; всяка стъпка попълва собствения prompt със свежите резултати от предишните стъпки и връща JSON, който се подава нататък. Финалният UI JSON идва от `step5_frontend_report_bg.txt` и използва данните от всички предходни етапи.【F:src/lib/multi-step-pipeline.ts†L57-L153】【F:steps/STEP5_frontend_report_bg.txt†L1-L126】
- **Защо самите prompt-ове не са в панела с настройки?** В момента `pipelinePromptCatalog` зарежда файловете от `steps/` като вътрешен ресурс и `getPromptSummaries()` ги дава само на orchestrator-а и логовете. Няма UI hook, който да визуализира `source`/`checksum` в admin панела – затова стъпките не се виждат там. Ако искаш контрол през UI, добави секция в admin екрана, която чете `getPromptSummaries()` и позволява смяна на каталога (напр. A/B превключвател).【F:src/lib/pipeline-prompts.ts†L1-L44】【F:FRONTEND_ANALYSIS_PROMPTS_BG.md†L22-L36】
