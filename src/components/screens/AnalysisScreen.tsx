...
import type { QuestionnaireData, IrisImage, AnalysisReport, IrisAnalysis, AIModelConfig, Recommendation, SupplementRecommendation, AIModelStrategy, IridologyManual, AIPromptTemplate } from '@/types'
...
export default function AnalysisScreen({
  ...
  const [aiStrategy] = useKVWithFallback<AIModelStrategy>('ai-model-strategy', {
    manualWeight: 40,
    promptWeight: 30,
...
Върни само ТЕКСТ на БЪЛГАРСКИ език, без JSON.`
      ...
      const patientData = `
ПАЦИЕНТ:
възр=${questionnaire.age}г|пол=${genderName}|BMI=${bmi}|тегло=${questionnaire.weight}кг|височ=${questionnaire.height}см
цели=${goalsText}|оплаквания=${complaintsText}
диета=${questionnaire.dietaryHabits.join(',')}|стрес=${questionnaire.stressLevel}|сън=${questionnaire.sleepHours}ч(${questionnaire.sleepQuality})
активност=${questionnaire.activityLevel}|медикам=${questionnaire.medications}|алергии=${questionnaire.allergies}
здраве=${questionnaire.healthStatus.join(',')}
`
      
      // Build the weighted prompt based on strategy
      ...
      basePromptContent = basePromptContent
        ...
        .replace(/\{\{sleepHours\}\}/g, String(questionnaire.sleepHours))
        ...
        .replace(/\{\{isRight\}\}/g, side === 'right' ? 'да' : 'не')
        .replace(/\{\{isLeft\}\}/g, side === 'left' ? 'да' : 'не')
