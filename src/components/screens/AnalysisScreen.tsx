...
import type { QuestionnaireData, IrisImage, AnalysisReport, IrisAnalysis, AIModelConfig, Recommendation, SupplementRecommendation, IridologyManual, AIPromptTemplate } from '@/types'
...
Върни само ТЕКСТ на БЪЛГАРСКИ език, без JSON.`
      ...
      const prompt = replaceTemplateTokens(promptTemplateContent, {
        side: sideName,
        imageHash,
        ...
        goals: goalsText || 'Няма',
        healthStatus: questionnaire.healthStatus?.join(', ') || 'Няма',
        complaints: complaintsText,
        dietaryHabits: questionnaire.dietaryHabits?.join(', ') || 'Няма',
        stressLevel: questionnaire.stressLevel || 'Няма данни',
        sleepHours: questionnaire.sleepHours ? String(questionnaire.sleepHours) : 'Няма данни',
        sleepQuality: questionnaire.sleepQuality || 'Няма данни',
        activityLevel: questionnaire.activityLevel || 'Няма данни',
        medications: questionnaire.medications || 'няма',
        allergies: questionnaire.allergies || 'няма',
        knowledgeContext
      })
