...
import manualGuide from '../../../manual.json?raw'
import analysisPromptTemplate from '../../../prompt.json?raw'

const buildPromptFromTemplate = (template: string, values: Record<string, string>) => {
...
💡 С собствен API ключ няма да имате rate limit проблеми.`
  ...
  const analyzeIris = async (
    ...
    try {
      ...
      const healthStatusText = questionnaire.healthStatus?.join(', ') || 'няма данни'
      const dietaryHabitsText = questionnaire.dietaryHabits?.join(', ') || 'няма данни'
      const sleepQualityLabel = {
        poor: 'лош',
        fair: 'среден',
        good: 'добър',
        excellent: 'отличен'
      }[questionnaire.sleepQuality] || questionnaire.sleepQuality
      const activityLabel = {
        sedentary: 'заседнал',
        light: 'лека активност',
        moderate: 'умерена активност',
        active: 'активен',
        'very-active': 'много активен'
      }[questionnaire.activityLevel] || questionnaire.activityLevel
      const stressLabel = {
        low: 'ниско',
        moderate: 'умерено',
        high: 'високо',
        'very-high': 'много високо'
      }[questionnaire.stressLevel] || questionnaire.stressLevel
      const medicationsText = questionnaire.medications || 'няма'
      const allergiesText = questionnaire.allergies || 'няма'

      const imageHash = `${side}-${hashString(iris.dataUrl)}`
...
Върни само ТЕКСТ на БЪЛГАРСКИ език, без JSON.`
        ...
        try {
          ...
          diagnosticText = diagnosticResponse
      ...
      if (!manualGuide.trim()) {
        throw new Error('Наръчникът manual.json не е зареден')
      }

      if (!analysisPromptTemplate.trim()) {
        throw new Error('Шаблонът prompt.json не е зареден')
      }

      addLog('info', 'Използване на manual.json като основен наръчник...')
      const knowledgeContext = manualGuide.trim()
      addLog('success', `Наръчник зареден (${knowledgeContext.length} символа)`)

      addLog('info', 'Подготовка на prompt от prompt.json...')
      let prompt = buildPromptFromTemplate(analysisPromptTemplate, {
        side: sideName,
        imageHash,
        ...
        dietaryHabits: dietaryHabitsText,
      ...
      const hasFindings = result.zones.some(z => z.status !== 'normal') || (result.artifacts?.length ?? 0) > 0

      if (!hasFindings) {
        addLog('warning', 'LLM върна само нормални зони - повторен анализ с фокус върху находките')
        console.log(`⚠️ [ИРИС ${side}] Няма отбелязани находки, стартира повторен анализ`)

        const reinforcement = `\nКРИТИЧНО: Предходният ти отговор НЕ съдържаше находки. Провери внимателно за лакуни, крипти, пигменти, радиални линии, ANW/лимфни пръстени и текстурни аномалии.\nНе връщай всички зони като "normal", освен ако ирисът е напълно хомогенен. Ако някои области са трудно четими поради качество, маркирай ги със status="attention" и опиши ограниченията.`

        if (diagnosticText) {
          prompt += `\nДИАГНОСТИЧНИ НАБЛЮДЕНИЯ (за сверка): ${diagnosticText}`
        }

        const fallbackPrompt = `${prompt}${reinforcement}`

        addLog('info', 'Повторно изпращане към LLM с изискване за видими находки...')
        const fallbackResponse = await callLLMWithRetry(fallbackPrompt, true, 1, imageForAnalysis)
        addLog('success', `Повторен отговор (${fallbackResponse.length} символа)`)

        const fallbackParsed = await robustJSONParse(fallbackResponse, `ИРИС ${side} (повторен анализ)`)
        const fallbackResult = {
          side,
          ...fallbackParsed.analysis,
          artifacts: fallbackParsed.analysis?.artifacts ?? [],
          zones: fallbackParsed.analysis?.zones ?? []
        }

        addLog('success', `Повторен анализ завършен: ${fallbackResult.zones.length} зони, ${fallbackResult.artifacts.length} артефакта`)
        console.log(`✅ [ИРИС ${side}] Повторен резултат:`, fallbackResult)
