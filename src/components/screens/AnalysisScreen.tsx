...
import type { IrisUnwrapResult } from '@/hooks/useIrisUnwrap'
...
export default function AnalysisScreen({
  ...
  // Refs hold the unwrap results that performAnalysis reads.
  // Using refs (not state) avoids the stale-closure problem: performAnalysis
  // is defined on the initial render but called later, so it must read the
  // latest values via a mutable ref rather than a captured state snapshot.
  const leftUnwrapRef = useRef<IrisUnwrapResult | null>(null)
  ...
  useEffect(() => {
    ...
    const loadConfigAndStartAnalysis = async () => {
      try {
        ...
        // Await iris unwrapping so the linearised map is ready before the AI pipeline runs.
        // Results are stored in refs (not relied upon via React state) to avoid the
        // stale-closure problem: performAnalysis closes over the initial render's state.
        if (unwrapBackendAvailable && !unwrapCalledRef.current) {
          ...
          try {
            const { left, right } = await unwrapImages(leftIris.dataUrl, rightIris.dataUrl)
            leftUnwrapRef.current = left
            rightUnwrapRef.current = right
            const found = (left?.found ? 1 : 0) + (right?.found ? 1 : 0)
            if (found > 0) {
              addLog('success', `✅ Разгъвката завърши (открити: ${found}) – линейна карта ще се изпрати към AI`)
            } else {
              addLog('warning', '⚠️ Разгъвката завърши, но зеницата не е открита – ще се използва оригиналното изображение')
            }
          } catch (e) {
  ...
  const performAnalysis = async () => {
    ...
    try {
      ...
      if (usePipelineV9) {
        ...
        const luw = leftUnwrapRef.current
        leftAnalysis = await executeV9Pipeline(
          ...
          luw?.found ? toMapDataUrl(luw.map_base || luw.mapped) : undefined,
          luw?.found ? toMapDataUrl(luw.map_structure) : undefined,
          luw?.found ? toMapDataUrl(luw.map_pigment) : undefined
        )
        ...
        const ruw = rightUnwrapRef.current
        rightAnalysis = await executeV9Pipeline(
          ...
          addLog,
          finalPipelineConfig,  // Pass pipeline config from admin panel
          ruw?.found ? toMapDataUrl(ruw.map_base || ruw.mapped) : undefined,
          ruw?.found ? toMapDataUrl(ruw.map_structure) : undefined,
          ruw?.found ? toMapDataUrl(ruw.map_pigment) : undefined
        )
      ...
      const report: AnalysisReport = {
        ...
        ...(leftUnwrapRef.current?.found && leftUnwrapRef.current.map_base ? {
          leftIrisMaps: {
            base:      leftUnwrapRef.current.map_base,
            structure: leftUnwrapRef.current.map_structure,
            pigment:   leftUnwrapRef.current.map_pigment,
          }
        } : {}),
        ...(rightUnwrapRef.current?.found && rightUnwrapRef.current.map_base ? {
          rightIrisMaps: {
            base:      rightUnwrapRef.current.map_base,
            structure: rightUnwrapRef.current.map_structure,
            pigment:   rightUnwrapRef.current.map_pigment,
          }
        } : {}),
      }
...
💡 С собствен API ключ няма да имате rate limit проблеми.`
  ...
  const analyzeIris = async (
    ...
    try {
      ...
      const unwrapResult = side === 'left' ? leftUnwrapRef.current : rightUnwrapRef.current
      ...
      if (enableDiagnostics) {
        ...
        const usingUnwrappedDiag = imageForAnalysis !== iris.dataUrl
        const diagnosticPrompt = (window.spark.llmPrompt as unknown as (strings: TemplateStringsArray, ...values: any[]) => string)`Ти си експертен иридолог. Получаваш ${usingUnwrappedDiag ? 'РАЗГЪНАТА (полярна→правоъгълна) карта на' : 'чисто изображение на'} ${sideName} ирис БЕЗ топографска карта.${usingUnwrappedDiag ? '\nКООРДИНАТНА СИСТЕМА: X-ос = минута 0–60 (по часовник от 12ч), Y-ос = пръстен R0 (зеница) → R11 (външен ирис).' : ''}
...
2. ВИДИМИ СТРУКТУРНИ НАХОДКИ в ириса:
   ...
   - Виждаш ли радиални линии${usingUnwrappedDiag ? ' (вертикални ивици в разгъвката)' : ' (от центъра навън)'}?
   - Виждаш ли стрес-пръстени${usingUnwrappedDiag ? ' (хоризонтални дъги/ленти в разгъвката)' : ' (концентрични кръгове около зеницата)'}?
...
`
      ...
      const templateVariables: Record<string, string> = {
        ...
        coordinateSystemDesc: usingUnwrapped
...
Y-ос (вертикална) = ПРЪСТЕН (RING), стойности R0 → R11
  ...
  - Мислено раздели ириса на 12 радиални сектора (min 0–5, 5–10, … 55–60)`,
        canvasIgnoreNote: usingUnwrapped
  ...
  - Само ирисовата тъкан в решетката съдържа информация`
      ...
      if (hasTemplateVariables) {
        ...
        const usingUnwrappedFallback = imageForAnalysis !== iris.dataUrl
        prompt = (window.spark.llmPrompt as unknown as (strings: TemplateStringsArray, ...values: any[]) => string)`Ти си опитен иридолог с 20 години клинична практика. Ще ти предоставя изображение на ${sideName} ирис и данни от пациента.

${usingUnwrappedFallback
  ? `⚠️ ВАЖНО: Получаваш РАЗГЪНАТА (полярна→правоъгълна) карта на ириса.
КООРДИНАТНА СИСТЕМА: X-ос = минута 0–60 (по часовник от 12ч: 0=12ч, 15=3ч, 30=6ч, 45=9ч), Y-ос = пръстен R0 (граница зеница) → R11 (външен ирис).
Зони се описват с minute_start/minute_end и ring_start/ring_end. Артефакти – с minute (0-60) и ring (0-11).`
  : `⚠️ ВАЖНО: Получаваш ОРИГИНАЛНО кръгово изображение на ириса БЕЗ наложени линии.
КООРДИНАТНА СИСТЕМА: minute=0–60 по часовник от 12ч (0=12ч, 15=3ч, 30=6ч, 45=9ч), ring=R0–R11 (от зеница навън). angle=[minute_start*6, minute_end*6].`}
...
- Радиални знаци: ${usingUnwrappedFallback ? 'вертикални ивици в разгъвката' : 'линии от центъра навън'}
- Стрес-пръстени (автономен пръстен): ${usingUnwrappedFallback ? 'хоризонтални дъги/ленти в разгъвката' : 'концентрични кръгове около зеницата'}
...
1. ВИДИМА НАХОДКА + СЪОТВЕТЕН СИМПТОМ → status:"concern"
2. ВИДИМА НАХОДКА БЕЗ СИМПТОМ → status:"attention"
3. НЯМА НАХОДКА → status:"normal"

ЗАДАЧА: Анализирай ${sideName} ирис и върни JSON. ЗОНИ (12 зони):
Зона 1 – мин:0-5,  Мозък/Нервна система
Зона 2 – мин:5-10, Хипофиза/Ендокринна
Зона 3 – мин:10-15, Щитовидна жлеза/Ендокринна
Зона 4 – мин:15-20, Белодробна${side==='right'?' (Д)':''}
Зона 5 – мин:20-25, Черен дроб/Детоксикация
Зона 6 – мин:25-30, Стомах/Храносмилателна
Зона 7 – мин:30-35, Панкреас/Храносмилателна
Зона 8 – мин:35-40, Бъбреци/Урогенитална
Зона 9 – мин:40-45, Надбъбречни жлези/Ендокринна
Зона 10 – мин:45-50, Сърце/Сърдечно-съдова${side==='left'?' (Л)':''}
Зона 11 – мин:50-55, Далак/Имунна
Зона 12 – мин:55-60, Лимфна система/Имунна

За всяка зона: id, name, organ, status, findings (max 80 симв), minute_start, minute_end, ring_start=0, ring_end=11, angle=[minute_start*6, minute_end*6] (backward-compat, degrees 0-360).

АРТЕФАКТИ (0 или повече):
type ∈ {"лакуна","крипта","пигмент","радиална линия","автономен пръстен"}
location="мин:XX-XX ринг:RX-RY", clock_pos="hh:mm-hh:mm", minute, ring, description (max 60 симв), severity.

ФОРМАТ НА ОТГОВОРА: САМО валиден JSON, БЕЗ markdown, БЕЗ нови редове в strings, САМО БЪЛГАРСКИ.

{
  "analysis": {
    "zones": [
      {"id":1,"name":"Мозъчна зона","organ":"Мозък","status":"normal","findings":"Визуално чиста зона","minute_start":0,"minute_end":5,"ring_start":0,"ring_end":11,"angle":[0,30]},
      {"id":2,"name":"Хипофизна зона","organ":"Хипофиза","status":"attention","findings":"Лека дисколорация","minute_start":5,"minute_end":10,"ring_start":0,"ring_end":11,"angle":[30,60]}
    ],
    "artifacts": [
      {"type":"лакуна","location":"мин:10-12 ринг:R2-R4","clock_pos":"2:00-2:30","minute":11,"ring":3,"description":"Малка тъмна лакуна","severity":"low"}
    ],
    "overallHealth": 75,
    "systemScores": [
      {"system":"Храносмилателна","score":70,"description":"Умерени находки"},
