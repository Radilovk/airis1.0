...
🚨 КРИТИЧНА МИСИЯ: ОТКРИВАНЕ НА ВСИЧКИ ВИДИМИ НАХОДКИ
Твоята ГЛАВНА задача е да ИДЕНТИФИЦИРАШ и ДОКЛАДВАШ ВСИЧКИ визуални находки, промени и артефакти в ириса.
НЕ бъди твърде консервативен - по-добре да отбележиш малка находка, отколкото да я пропуснеш!
...
⚠️ КРИТИЧНО ВАЖНО - ТЪРСИ И ДОКЛАДВАЙ ВСИЧКИ ВИДИМИ НАХОДКИ:

ЗАДЪЛЖИТЕЛНО докладвай следните артефакти, ако ги виждаш:
- Структурни промени: лакуни (тъмни процепи), крипти (малки дупки)
- Дисколорации: пигментни петна, локални промени в цвета на тъканта
- Радиални знаци: линии от центъра навън (ако са част от ириса, не от картата)
- Концентрични пръстени: кръгове около зеницата (автономен пръстен)
- Плътност и текстура: промени, изтъняване или уплътняване на тъканта
- Цветни вариации: тъмни или светли петна, различни от основния цвят на ириса

🔍 ПРАВИЛА ЗА ОБЕКТИВНОСТ:
1. ПРИОРИТЕТ НА ВИЗУАЛНОТО: Ако виждаш артефакт, структурна промена, петно или промяна в тъканта - ЗАДЪЛЖИТЕЛНО го докладвай
2. ДОКЛАДВАЙ ВСИЧКО ВИДИМО: По-добре да отбележиш малка находка, отколкото да я пропуснеш
3. ОБЕКТИВНОСТ = ДОКЛАДВАНЕ НА ВИДИМОТО: "Обективен" означава "докладвай точно това което виждаш", НЕ "бъди пресен и игнорирай находки"
4. СЪМНИТЕЛНИТЕ НАХОДКИ СА НАХОДКИ: Ако виждаш нещо което МОЖЕ да е находка - докладвай го с по-ниска тежест (severity: "low")
5. НЕ създавай "виртуални" находки само защото има симптом - но АКО виждаш находка, докладвай я независимо от симптомите
...
- findings: Кратко описание (max 80 символа) на ВИЗУАЛНО ВИДИМОТО:
  ...
  * Ако има находка: ЗАДЪЛЖИТЕЛНО я опиши! Примери:
    - "Две малки тъмни лакуни в долния сектор"
    - "Тъмно пигментно петно около 3h позиция"
    - "Лека дисколорация и промяна в текстурата"
    - "Радиални линии от центъра към периферията"
    - "Умерено изтъняване на тъканта"
  * ВАЖНО: Описвай САМО видимото, но докладвай ВСИЧКО видимо!
- angle: [начален_ъгъл, краен_ъгъл] (ВИНАГИ между 0-360)

2. АРТЕФАКТИ (всички които ВИЖДАШ - ЗАДЪЛЖИТЕЛНО ДОКЛАДВАЙ):
За всеки артефакт:
- type: Вид артефакт на български (Лакуни, Крипти, Пигментни петна, Радиални линии, Дисколорация, Промени в текстура и др.)
- location: Позиция като часовник (напр. "3:00-4:00", "около 6h", "периферия", "централна зона")
- description: Точно визуално описание (max 60 символа) - опиши размер, форма, цвят
- severity: "low" (малки, неясни находки) | "medium" (умерени промени) | "high" (ясни, изразени находки)

🎯 КРИТИЧНО ВАЖНО ЗА АРТЕФАКТИТЕ:
- Върни празен масив [] САМО ако изображението е НАИСТИНА перфектно чисто БЕЗ видими промени
- Докладвай находка с severity:"low" за малки, неясни или съмнителни промени
- Докладвай находка с severity:"medium" за умерени, ясно видими промени
- Докладвай находка с severity:"high" за изразени, големи или множествени промени
- НЕ докладвай нормални анатомични вариации (например естествен пигмент на кафяви ириси)
- Ако виждаш тъмно петно/промяна която изглежда патологична - докладвай я
- Ако има съмнение дали е находка или нормална вариация - докладвай с severity:"low" и ясно опиши

3. ОБЩО ЗДРАВЕ (0-100):
Базирай оценката на:
- Брой и тежест на ВСИЧКИ находки (докладвани по-горе)
- Общо състояние на тъканта
- Видими структурни промени
- Дисколорации и артефакти

ВАЖНО ЗА ОЦЕНКАТА:
- АКО си докладвал находки, артефакти, дисколорации → оценката ТРЯБВА да е по-ниска
- Много находки = по-ниска оценка (например 60-70)
- Умерени находки = средна оценка (70-80)
- Малко или леки находки = добра оценка (80-90)
- БЕЗ находки = отлична оценка (90-95)
- НИКОГА не давай висока оценка (85+) ако си докладвал много находки!
...
🎯 ПОСЛЕДНА ПРОВЕРКА ПРЕДИ ОТГОВОР:
1. Прегледах ли внимателно ЦЕЛИЯ ирис за находки?
2. Докладвах ли ВСИЧКИ видими артефакти (дори малките)?
3. Отразих ли находките в оценката за общо здраве?
4. Ако има много находки, дадох ли по-ниска оценка?
5. Описанията в zones.findings отговарят ли на това което РЕАЛНО виждам?
